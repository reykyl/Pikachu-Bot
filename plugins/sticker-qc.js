...
text = m.quoted.text;
} else return conn.reply(m.chat, `*ᰔᩚ ( ᜊ°-° )ᜊ  .ᐟ .ᐟ 🧃* *_Usa correctamente el comando entrenador.⚡_*`, m, fake);
if (!text) return conn.reply(m.chat, `*ᰔᩚ ( ᜊ°-° )ᜊ  .ᐟ .ᐟ 🧃* *_Usa correctamente el comando entrenador.⚡_*`, m, fake);
const who = m.mentionedJid && m.mentionedJid[0] ? m.mentionedJid[0] : m.fromMe ? conn.user.jid : m.sender; 
const mentionRegex = new RegExp(`@${who.split('@')[0].replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*`, 'g');
const mishi = text.replace(mentionRegex, '');
if (mishi.length > 40) return conn.reply(m.chat, `*ᰔᩚ ( ᜊ°-° )ᜊ  .ᐟ .ᐟ 🧃* _*Entrenador! El texto no puede tener más de 30 palabras.*_`, m, fake);

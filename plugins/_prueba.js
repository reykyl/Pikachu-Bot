...
var handler = async (m, { conn, args }) => {
    if (!args[0]) {
        return conn.reply(m.chat, `Por favor, envía un enlace de TikTok para obtener la miniatura.`, m);
    }

    try {
        await conn.reply(m.chat, `Obteniendo la miniatura del video...`, m);

        const tiktokData = await tiktokdl(args[0]);

        if (!tiktokData || !tiktokData.thumbnail) {
            return conn.reply(m.chat, "No se pudo obtener la miniatura del video.", m);
        }
...
`.trim();
        ...
        return conn.reply(m.chat, `Ocurrió un error al obtener la miniatura: ${error1.message}`, m);
    }
};
